...
<section class="py-20 bg-amber-50/50">
    <div class="container mx-auto px-6">
        <div class="text-center mb-16">
            ...
            <p class="text-gray-600 max-w-2xl mx-auto">Cerita manis dari pelanggan setia Honey Mood yang telah merasakan kemurnian madu asli kami.</p>
        </div>

        <div class="flex overflow-x-auto pb-8 gap-6 snap-x snap-mandatory hide-scrollbar">
              @foreach($testimonies as $item)
             <div class="min-w-[320px] md:min-w-[384px] snap-center bg-white p-8 rounded-2xl shadow-sm border border-amber-100 hover:shadow-md transition-shadow duration-300 relative">
                    ...
                    <div class="flex items-center gap-4">
                        @if($item->photo)
                            <img src="{{ asset('storage/' . $item->photo) }}" alt="{{ $item->name }}" class="w-12 h-12 rounded-full object-cover border-2 border-amber-200">
                        @else
                        ...
                        @endif
                        <div>
                            <h4 class="font-bold text-gray-900 leading-none mb-1">{{ $item->name }}</h4>
                            <span class="text-sm text-gray-500">{{ $item->job }}</span>
                        </div>
                    </div>
...
<style>
/* Sembunyikan scrollbar tapi fungsi scroll tetap jalan */
